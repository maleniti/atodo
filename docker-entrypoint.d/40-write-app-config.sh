...
# LANDING_VIDEO_URL, and API_BASE_URL env vars so an admin can set/rotate them
# per-deployment (docker run -e / compose environment:) without rebuilding the
# image. Left empty if an env var isn't set -- app.js's loadUnsplashAccessKey()
# then falls back to a per-device key entered through the background picker
# itself (same as local dev without a config.js), landing.js just shows its
# video placeholder instead of an embed, and auth.js's apiFetch falls back to
# a same-origin /atodo/v1.
set -eu
...
// LANDING_VIDEO_URL/API_BASE_URL env vars -- see
// docker-entrypoint.d/40-write-app-config.sh. Not persisted anywhere; edits
// here are lost on the next container start.
window.APP_CONFIG = { unsplashAccessKey: "${escaped_key}", landingVideoUrl: "${escaped_video_url}", apiBaseUrl: "${escaped_api_base_url}" };
JS
